 ...
 */
package uk.ac.babraham.conclave.DataWriters;
...
import uk.ac.babraham.conclave.DataTypes.Genome.AnnotationTagValue;
import uk.ac.babraham.conclave.DataTypes.Genome.Chromosome;
import uk.ac.babraham.conclave.DataTypes.Genome.CoreAnnotationSet;
...
import uk.ac.babraham.conclave.Dialogs.Cancellable;
 ...
 */
public class ConclaveDataWriter implements Runnable, Cancellable {
	 ...
	 * @param file the file
	 */
	public void writeData (ConclaveApplication application, File file) {
	...
	public void run() {
		try {
			...
			printReplicateSets(dataSets, replicateSets, p);
			...
			if (visibleStores != null) {
				printVisibleDataStores(dataSets, replicateSets, p);
			}
	 ...
	 */
	private void printReplicateSets (DataSet [] dataSets, ReplicateSet [] replicates, PrintStream p) {
	 ...
	 * @return false if cancelled, else true
	 */
	private boolean printProbeSet (ProbeSet probeSet, Probe [] probes, DataSet [] dataSets, PrintStream p) throws IOException {
	 ...
	 * @param p the p
	 */
	private void printVisibleDataStores (DataSet [] dataSets, ReplicateSet [] replicates, PrintStream p) {
	 ...
	 * @param p the p
	 */
	private boolean printProbeLists (Probe [] probes, PrintStream p) throws ConclaveException,IOException {
		...
		for (int i=1;i<orderedProbes.length;i++) {
			if (orderedProbeIndices[i] != orderedProbes[i].length) {
				throw new ConclaveException("Probe list "+i+" only reported "+orderedProbeIndices[i]+" out of "+orderedProbes[i].length+" probes");
			}
		}
