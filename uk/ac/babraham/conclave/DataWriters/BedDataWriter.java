...
import uk.ac.babraham.conclave.Utilities.ValidFileNameGenerator;
import uk.ac.babraham.conclave.Utilities.FileFilters.BEDFileFilter;
...
public class BedDataWriter implements Runnable, Cancellable {
	...
	public void startProcessing () {
		...
		if (file.exists()) {
			...
			int answer = JOptionPane.showOptionDialog(ConclaveApplication.getInstance(),file.getName()+" exists.  Do you want to overwrite the existing file(s)?","Overwrite file?",0,JOptionPane.QUESTION_MESSAGE,null,new String [] {"Overwrite and Save","Cancel"},"Overwrite and Save");
	...
	public void run() {
		...
		Chromosome [] chrs = ConclaveApplication.getInstance().dataCollection().genome().getAllChromosomes();
