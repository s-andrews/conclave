 ...
 */
package uk.ac.babraham.conclave.Network;
...
public class UpdateChecker {
	 ...
	 * @throws ConclaveException if we were unable to check for an update
	 */
	public static boolean isUpdateAvailable () throws ConclaveException {
		if (latestVersion == null) {
			getLatestVersionNumber();
		}
		return UpdateChecker.isNewer(ConclaveApplication.VERSION, latestVersion);
	}
	 ...
	 * @throws ConclaveException if the remote version couldn't be retrieved
	 */
	public static String getLatestVersionNumber () throws ConclaveException  {
	
		try {
			
			URL updateURL = new URL("https","www.bioinformatics.babraham.ac.uk","/projects/conclave/current_version.txt");
			...
			e.printStackTrace();
			throw new ConclaveException("Couldn't contact the update server to check for updates");
		}
	}
