 ...
 */
package uk.ac.babraham.conclave.Reports;
	...
	public void run() {
		...
		for (int c=0;c<chrs.length;c++) {
			...
			for (int p=0;p<probes.length;p++) {
				...
				if (end - start < 100) {
					progressWarningReceived(new ConclaveException("View for "+probes[p].name()+" was too small to export"));
					continue;
				}
				...
				try {
					ImageSaver.savePNG(ConclaveApplication.getInstance().chromosomeViewer(), saveFile);
				} 
				catch (IOException e) {
