...
public abstract class ProbeFilter implements Runnable,Cancellable {
	 ...
	 */
	public ProbeFilter (DataCollection collection) throws ConclaveException {
		if (! collection.isQuantitated()) {
			throw new ConclaveException("You must quantitate your data before running filters.");
		}
		this.collection = collection;
		startingList = collection.probeSet().getActiveList();
	}
	
	public ProbeFilter (DataCollection collection,ProbeList startingList) throws ConclaveException {
		if (! collection.isQuantitated()) {
			throw new ConclaveException("You must quantitate your data before running filters.");
		}
		this.collection = collection;
	 ...
	 * 
	 * @throws ConclaveException if the filter is not ready to run.
	 */
