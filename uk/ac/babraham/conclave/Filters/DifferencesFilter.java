...
public class DifferencesFilter extends ProbeFilter {
	 ...
	 */
	public DifferencesFilter (DataCollection collection) throws ConclaveException {
		super(collection);
	}
	...
	protected void generateProbeList() {
		...
		PROBE: for (int p=0;p<probes.length;p++) {
			...
			for (int s=0;s<allStores.length;s++) {
				...
				catch (ConclaveException sme){continue;}
			}
			...
			for (int fromIndex=0;fromIndex<fromStores.length;fromIndex++) {
				for (int toIndex=0;toIndex<toStores.length;toIndex++) {
					...
					case MINIMUM:
						...
						break;
					default:
						progressExceptionReceived(new ConclaveException("Unknown difference type "+differenceType));
					}
