 ...
 */
package uk.ac.babraham.conclave.Filters.Variance;
...
import uk.ac.babraham.conclave.Analysis.Statistics.IndexTTestValue;
import uk.ac.babraham.conclave.Analysis.Statistics.SmoothedVarianceDataset;
...
import uk.ac.babraham.conclave.Displays.VariancePlot.VariancePlotPanel;
...
public class VarianceIntensityDifferenceFilter extends ProbeFilter {
	 ...
	 */
	public VarianceIntensityDifferenceFilter (DataCollection collection) throws ConclaveException {
		super(collection);
	...
	protected void generateProbeList() {
		...
		if (keepers.size() != probes.length) {
			...
			progressWarningReceived(new ConclaveException("Excluded "+(probes.length - keepers.size())+" probes which contained null values"));
	...
	private class VarianceIntensityOptionsPanel extends JPanel implements ListSelectionListener, KeyListener, ItemListener {
		...
		public VarianceIntensityOptionsPanel () {
			...
			DataStore [] drawnStores = ConclaveApplication.getInstance().drawnDataStores();
