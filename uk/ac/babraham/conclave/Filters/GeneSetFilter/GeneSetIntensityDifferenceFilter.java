...
import uk.ac.babraham.conclave.Analysis.Statistics.BenjHochFDR;
import uk.ac.babraham.conclave.Analysis.Statistics.MappedGeneSetTTestValue;
import uk.ac.babraham.conclave.Analysis.Statistics.SimpleStats;
import uk.ac.babraham.conclave.Analysis.Statistics.TTest;
...
import uk.ac.babraham.conclave.Dialogs.Renderers.TypeColourRenderer;
import uk.ac.babraham.conclave.Filters.ProbeFilter;
import uk.ac.babraham.conclave.GeneSets.GeneSetCollection;
import uk.ac.babraham.conclave.GeneSets.GeneSetCollectionParser;
import uk.ac.babraham.conclave.GeneSets.MappedGeneSet;
...
//	private boolean validGeneSetFile = false;
	 ...
	 * @throws ConclaveException if the collection isn't quantitated
	 */
	public GeneSetIntensityDifferenceFilter (DataCollection collection) throws ConclaveException {
		super(collection);
		...
		if (!(startingList instanceof ProbeSet)) {
			JOptionPane.showMessageDialog(ConclaveApplication.getInstance(), "<html>This test requires a representative set of all probes to be valid.<br>Be careful running it on a biased subset of probes</html>", "filtered list used", JOptionPane.WARNING_MESSAGE);
		}
	...
	protected void generateProbeList() {
		...
		try {
			...
			/* or if we're using existing probelists, create mappedGeneSets from them */
				...
				throw new ConclaveException("Haven't got any genesets to use, shouldn't have got here without having any selected.");		
			}
			
			if(mappedGeneSets == null || mappedGeneSets.length == 0){
				throw new ConclaveException("Couldn't map gene sets to the probes, try again with a different probe set");				
			}		
	...
	public void runFilter () throws ConclaveException {
		if (! isReady()) {
			throw new ConclaveException("Filter is not ready to run");
		}
	...
	*/		/* get selected probelists  */	
			if(optionsPanel.probeListRadioButton.isSelected()){
				...
				if (probeLists == null || probeLists.length == 0){
					...
					JOptionPane.showMessageDialog(ConclaveApplication.getInstance(), "No probelists were selected.",  "No probe lists selected", JOptionPane.INFORMATION_MESSAGE);
					progressCancelled();
					optionsChanged();
					return;
				}			
...
//		private JCheckBox calculateLinearRegressionBox;
		...
		public DifferencesOptionsPanel () {
			...
			fromStores=ConclaveApplication.getInstance().drawnDataStores();
			...
			toStores=ConclaveApplication.getInstance().drawnDataStores();
...
//				System.err.println("adjusting the z-score threshold to " + zScoreThreshold);
		...
		public void actionPerformed(ActionEvent ae) {
			...
			if (action.equals("select_file")) {
				...
				JFileChooser fc = new JFileChooser(ConclavePreferences.getInstance().getDataLocation());				
				...
				fc.setFileFilter(ff);				
				fc.setCurrentDirectory(ConclavePreferences.getInstance().getDataLocation());				
				...
				if(minGenesInSet > maxGenesInSet){
					JOptionPane.showMessageDialog(ConclaveApplication.getInstance(), "Minimum number of genes in set cannot be greater than maximum.", "Number of genes needs adjusting", JOptionPane.ERROR_MESSAGE);
					return;
				}		
				else if(validGeneSetFilepath == null){
					JOptionPane.showMessageDialog(ConclaveApplication.getInstance(), "A valid gene set file needs to be selected.", "Gene set file required", JOptionPane.ERROR_MESSAGE);
					return;
				}
				else{
					startRunningFilter();	
	...
	private boolean fileValid(String filepath){
		...
		if(f.exists() && !f.isDirectory()) {
			...
			JOptionPane.showMessageDialog(ConclaveApplication.getInstance(), "The gene set information file couldn't be found, please find a valid file to load.", "Couldn't load gene set file", JOptionPane.ERROR_MESSAGE);
			return false;
		}
	}	
