 ...
 */
public class ConclaveAnnotationReimportParser extends AnnotationParser {
	 ...
	 */
	public ConclaveAnnotationReimportParser (Genome genome) {
		super(genome);
	}	
...
//				break;
	...
	private void parseDataVersion (String [] sections) throws ConclaveException {
		...
		if (thisDataVersion > MAX_DATA_VERSION) {
			throw new ConclaveException("This data file needs a newer verison of SeqMonk to read it.");
		}
	}

	
	private void parseGenome (String [] sections, Genome genome) throws ConclaveException {
	 ...
	 */
	private AnnotationSet parseAnnotation (String [] sections, Genome genome) throws ConclaveException, IOException {
		...
		for (int i=0;i<featureCount;i++) {
			...
			try {
				...
				progressWarningReceived(new ConclaveException("Annotation feature could not be mapped to chromosome '"+sections[1]+"'"));
				continue;
			}
	...
	private class AnnotationSetSelector extends JDialog {
		...
		public AnnotationSetSelector (AnnotationSet [] annotationSets) {
			super(ConclaveApplication.getInstance(),"Select AnnotationSets to import");
