...
import uk.ac.babraham.conclave.Utilities.ChromosomeWithOffset;
...
public class GenericAnnotationParser extends AnnotationParser {
	...
	public AnnotationSet [] parseAnnotation (File file, Genome genome) throws Exception {
		...
		if (options == null) {
			options = new JDialog(ConclaveApplication.getInstance());
		...
		while ((line = br.readLine())!= null) {
			...
			if (maxIndexValue >= sections.length) {
				progressWarningReceived(new ConclaveException("Not enough data ("+sections.length+") to get a probe name on line '"+line+"'"));
			...
			try {
				...
				if (useStrand) {
					if (sections[strandColValue].equals("+") || sections[strandColValue].equals("1") || sections[strandColValue].equals("FF") || sections[strandColValue].equals("F")) {
						...
						progressWarningReceived(new ConclaveException("Unknown strand character '"+sections[strandColValue]+"' marked as unknown strand"));
				...
				progressWarningReceived(new ConclaveException("Location "+sections[startColValue]+"-"+sections[endColValue]+" was not an integer"));
				continue;
			}
			...
			if (end > c.chromosome().length()) {
				...
				progressWarningReceived(new ConclaveException("Reading position "+end+" was "+overrun+"bp beyond the end of chr"+c.chromosome().name()+" ("+c.chromosome().length()+")"));
				continue;
			}

			if (start < 1) {
				progressWarningReceived(new ConclaveException("Reading start position "+start+" was less than 1"));
				continue;						
			}
			...
			if (!addedTypes.contains(type)) {
				...
				if (addedTypes.size() > 100) {
					throw new ConclaveException("More than 100 different types of feature added - you don't want to do this!");
				}
			}
