...
public class GFF3AnnotationParser extends AnnotationParser {
	...
	public AnnotationSet [] parseAnnotation(File file, Genome genome, String prefix) throws Exception {
		...
		if (prefix == null) {
			featurePrefix = JOptionPane.showInputDialog(ConclaveApplication.getInstance(), "Feature prefix", "GFFv3/GTP Options", JOptionPane.QUESTION_MESSAGE);
		}
		else {
		...
		while ((line = br.readLine())!= null) {
			...
			if (sections.length < 7) {
				progressWarningReceived(new ConclaveException("Not enough data from line '"+line+"'"));
				continue;
			}
			...
			try {
				...
				progressWarningReceived(new ConclaveException("Location "+sections[3]+"-"+sections[4]+" was not an integer"));
				continue;
			}
			...
			try {
				...
				progressWarningReceived(new ConclaveException("Couldn't find a chromosome called "+sections[0]));
				continue;	
			}
...
//							System.out.println("Key='"+keyValue[0]+"' value='"+keyValue[1]+"'");
					...
					if (keyValue.length == 2) {
						...
						progressWarningReceived(new ConclaveException("No key value delimiter in "+attributes[a]));
					}
