...
import uk.ac.babraham.conclave.DataTypes.Genome.SingleGenome;
...
public class GenomeParser implements Runnable {
	...
	/** The prefs. */
	private ConclavePreferences prefs = ConclavePreferences.getInstance();
		...
		for (int g=0;g<singleGenomes.length;g++) {
			...
			if (cacheCompleteFile.exists()) {
				...
				try {
					...
					if (! ConclaveApplication.VERSION.equals(line)) {
						System.err.println("Version mismatch between cache ('"+line+"') and current version ('"+ConclaveApplication.VERSION+"') - reparsing");
						cacheFailed = true;
					}
				}
	 ...
	 */
	private boolean processAttributeReturnSkip (String attribute, Feature feature) throws ConclaveException {
		...
		if (nameValue[0].equals("location")) {
			...
			if (nameValue.length < 2) {
				throw new ConclaveException("Location didn't have an '=' delimiter");
			}
	 ...
	 */
	private Chromosome parseChromosome (BufferedReader br, SingleGenome genome) throws ConclaveException, IOException {
		String line;
		while ((line=br.readLine())!=null) {
			...
			if (line.startsWith("AC")) {
				...
				if (sections.length != 6) {
					...
					progressWarningReceived(new ConclaveException("AC line didn't have 6 sections '"+line+"'"));
					skipToEntryEnd(br);
					continue;
				}
			...
			if (line.startsWith("//")) {
				throw new ConclaveException("Couldn't find AC line");
			}
		}
		return null;
	}
