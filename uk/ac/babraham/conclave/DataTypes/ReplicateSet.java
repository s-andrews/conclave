...
public class ReplicateSet extends DataStore {
	...
	public float getValueForProbeExcludingUnmeasured(Probe p) throws ConclaveException {
	...
	public float [] getValuesForProbe (Probe p) throws ConclaveException {
	...
	public float getStDevForProbe (Probe p) throws ConclaveException {
		float [] values = getValuesForProbe(p);
		return SimpleStats.stdev(values);
	}
	
	public float getSEMForProbe (Probe p) throws ConclaveException {
		float [] values = getValuesForProbe(p);
		return (float)(SimpleStats.stdev(values)/Math.sqrt(values.length));
	}
	
	public float getCoefVarForProbe (Probe p) throws ConclaveException {
		float [] values = getValuesForProbe(p);
		return SimpleStats.stdev(values)/getValueForProbe(p);
	}

	public float getQuartileCoefDispForProbe (Probe p) throws ConclaveException {
		float [] values = getValuesForProbe(p);
	...
	public float getUnmeasuredCountForProbe (Probe p) throws ConclaveException {
		float [] values = getValuesForProbe(p);
