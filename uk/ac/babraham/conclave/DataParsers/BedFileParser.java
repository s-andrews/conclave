			...
			for (int f=0;f<probeFiles.length;f++) {
				...
				while ((line = br.readLine())!= null) {
					...
					try {
						...
						if (start > end) {
							progressWarningReceived(new ConclaveException("End position "+end+" was lower than start position "+start));
							int temp = start;
							start = end;
							end = temp;
						}
						...
						if (sections.length >= 6) {
							if (sections[5].equals("+")) {
								...
								progressWarningReceived(new ConclaveException("Unknown strand character '"+sections[5]+"' marked as unknown strand"));
						...
						progressWarningReceived(new ConclaveException("Location "+sections[0]+"-"+sections[1]+" was not an integer"));
						continue;
					}
					try {
