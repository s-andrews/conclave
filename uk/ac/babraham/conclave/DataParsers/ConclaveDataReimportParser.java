...
import uk.ac.babraham.conclave.DataTypes.Genome.MultiGenome;
 ...
 */
public class ConclaveDataReimportParser extends DataParser {
	 ...
	 */
	public ConclaveDataReimportParser (DataCollection collection) {
		super(collection);
	 ...
	 */
	private DataSet [] parseSamples (String [] sections) throws ConclaveException, IOException {
		if (sections.length != 2) {
				...
				for (int r=0;r<readCount;r++) {
					...
					if (dataSetsToParse[i]) {
						...
						try {
							...
							progressWarningReceived(new ConclaveException("Read from sample "+i+" could not be mapped to chromosome '"+sections[0]+"'"));
							continue;
						}
	 ...
	 * @throws ConclaveException
	 */
	private void parseGenome (String [] sections) throws ConclaveException {
		...
		if (!(genome.species().equals(sections[1]) && genome.assembly().replaceAll("_v\\d+$", "").equals(sections[2].replaceAll("_v\\d+$", "")))) {
			...
			throw new ConclaveException("Genome versions didn't match between SeqMonk files ("+sections[1]+" "+sections[2]+" vs "+genome.species()+" "+genome.assembly()+")");
		}	
	}
	...
	private class DataSetSelector extends JDialog {
		...
		public DataSetSelector (DataSet [] dataSets) {
			super(ConclaveApplication.getInstance(),"Select DataSets to import");
			setModal(true);
