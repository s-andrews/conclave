...
import uk.ac.babraham.conclave.DataTypes.Genome.AnnotationCollection;
...
import uk.ac.babraham.conclave.Displays.FeatureListViewer.FeatureListViewer;
 ...
 */
public class FindFeaturesByNameDialog extends JDialog implements ActionListener, Runnable, Cancellable {
	 ...
	 */
	public FindFeaturesByNameDialog (DataCollection dataCollection) {
		super(ConclaveApplication.getInstance(),"Find Feature Names...");
		setSize(700,350);
		...
		search = new JTextArea(3,15);
		choicePanel.add(new JScrollPane(search));
		...
		if (FindFeaturesByNameDialog.lastSearchedType.length() > 0) {
			for (int i=0;i<featureTypes.length;i++) {
				if (featureTypes[i].equals(FindFeaturesByNameDialog.lastSearchedType)) {
	...
	private void makeFeatureList () {
		...
		for (int i=0;i<inputTerms.length;i++) {
			if (inputTerms[i].trim().length() == 0) continue;
