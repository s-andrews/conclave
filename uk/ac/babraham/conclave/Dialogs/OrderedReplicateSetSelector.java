 ...
 */
package uk.ac.babraham.conclave.Dialogs;
...
/**
 * The Class OrderedReplicateSetSelector provides a means to select a set of
 * replicate sets in a specific order, initially for use in the PCA
 * dialog but we can probably use it elsewhere as well.
 */
public class OrderedReplicateSetSelector extends JDialog implements ActionListener, ListSelectionListener {
		
	/** The available set model. */
	private DefaultListModel availableSetsModel = new DefaultListModel();

	/** The available set list. */
	private JList availableSetsList;
	
	/** The final list of selected lists */
	private ReplicateSet [] orderedSets = null;
	 ...
	 */
	public OrderedReplicateSetSelector (JDialog parent, ReplicateSet [] allSets, ReplicateSet [] initiallySelectedSets) {
		super(parent,"Select Replicate Sets");
		setSize(600,400);
		setLocationRelativeTo(parent);
		setModal(true);
		
		if (initiallySelectedSets == null) initiallySelectedSets = new ReplicateSet[0];
		...
		availablePanel.add(new JLabel("Available Sets",JLabel.CENTER),c2);
		
		c2.gridy++;
		c2.weighty=1;
		c2.fill=GridBagConstraints.BOTH;

		availableSetsList = new JList(availableSetsModel);
		availableSetsList.addListSelectionListener(this);
		availablePanel.add(new JScrollPane(availableSetsList),c2);
		...
		// Populate the initially selected sets
		for (int i=0;i<initiallySelectedSets.length;i++) {
			usedModel.addElement(initiallySelectedSets[i]);
		}
		
		// Populate the available sets, but missing out the initially
		// selected lists
		AVAILABLE: for (int i=0;i<allSets.length;i++) {
			for (int j=0;j<initiallySelectedSets.length;j++) {
				if (allSets[i] == initiallySelectedSets[j]) {
					continue AVAILABLE;
				}
			}
			availableSetsModel.addElement(allSets[i]);
		}
		...
		availableSetsModel.addElement("temp");
		usedModel.addElement("temp");
		validate();
		availableSetsModel.removeElement("temp");
	...
	public void actionPerformed(ActionEvent ae) {
		...
		if (c.equals("add")) {
			Object [] adds = availableSetsList.getSelectedValues();
			for (int i=0;i<adds.length;i++) {
				usedModel.addElement(adds[i]);
				availableSetsModel.removeElement(adds[i]);
			}
		}
			...
			for (int i=0;i<removes.length;i++) {
				...
				availableSetsModel.addElement(removes[i]);
			}			
		}
		else if (c.equals("up")) {
			...
			orderedSets = new ReplicateSet [o.length];
			for (int i=0;i<o.length;i++) {
				orderedSets[i] = (ReplicateSet)o[i];
			}
			setVisible(false);
		}
	...
	public ReplicateSet [] getOrderedSets () {
		return orderedSets;
	}
	...
	public void valueChanged(ListSelectionEvent ae) {
		...
		if (availableSetsList.getSelectedIndices().length>0) {
