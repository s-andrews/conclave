 ...
 */
package uk.ac.babraham.conclave.Dialogs.CrashReporter;
...
import uk.ac.babraham.conclave.Displays.HTMLDisplay.HTMLDisplayDialog;
import uk.ac.babraham.conclave.Preferences.ConclavePreferences;
import uk.ac.babraham.conclave.R.RException;
...
//	public static final String reportURLString = "http://bilin1/cgi-bin/public/crashreporter.cgi";
	...
	public CrashReporterDialog (Throwable e) {
		super(ConclaveApplication.getInstance(),"Oops - Crash Reporter");
		...
		if (e instanceof OutOfMemoryError) {
			...
			JOptionPane.showMessageDialog(ConclaveApplication.getInstance(), "<html>You ran out of memory!<br><br>Please look at Help &gt; Contents &gt; Configuration to see how to fix this", "Out of memory", JOptionPane.ERROR_MESSAGE);				
			return;
		}
		...
		ImageIcon sadMonk = new ImageIcon(ClassLoader.getSystemResource("uk/ac/babraham/Conclave/Resources/sad_monk100.png"));
	...
	private class ReportSender extends JDialog implements ActionListener, Runnable {
		...
		public ReportSender (CrashReporterDialog cr) {
			...
			email.setText(ConclavePreferences.getInstance().getCrashEmail());
			...
			if (ConclavePreferences.getInstance().getCrashEmail().length() > 0) {
		...
		private String makeReportText () {
			...
			sb.append(ConclaveApplication.VERSION);
			sb.append("\n\n");
		...
		public void actionPerformed(ActionEvent ae) {
			if (ae.getActionCommand().equals("cancel")) {
				...
				if (rememberEmail.isSelected()) {
					ConclavePreferences.getInstance().setCrashEmail(email.getText());
					try {
