...
import uk.ac.babraham.conclave.DataParsers.DataParser;
import uk.ac.babraham.conclave.DataParsers.ConclaveParser;
import uk.ac.babraham.conclave.DataTypes.CacheListener;
...
import uk.ac.babraham.conclave.DataTypes.Genome.AnnotationCollectionListener;
...
import uk.ac.babraham.conclave.DataTypes.Probes.ProbeSetChangeListener;
import uk.ac.babraham.conclave.DataWriters.ConclaveDataWriter;
import uk.ac.babraham.conclave.Dialogs.GenomeSelector;
import uk.ac.babraham.conclave.Dialogs.ConclavePreview.ConclavePreview;
import uk.ac.babraham.conclave.Dialogs.ConclavePreview.ConclavePreviewPanel;
import uk.ac.babraham.conclave.Dialogs.DataParser.DataParserOptionsDialog;
...
import uk.ac.babraham.conclave.Displays.StatusPanel;
import uk.ac.babraham.conclave.Displays.ChromosomeViewer.ChromosomePositionScrollBar;
import uk.ac.babraham.conclave.Displays.ChromosomeViewer.ChromosomeViewer;
import uk.ac.babraham.conclave.Displays.DataViewer.DataViewer;
import uk.ac.babraham.conclave.Displays.GenomeViewer.GenomeViewer;
import uk.ac.babraham.conclave.Displays.Vistory.VistoryDialog;
import uk.ac.babraham.conclave.Displays.WelcomePanel.InitialSetupPanel;
import uk.ac.babraham.conclave.Displays.WelcomePanel.WelcomePanel;
import uk.ac.babraham.conclave.Menu.ConclaveMenu;
import uk.ac.babraham.conclave.Network.GenomeDownloader;
import uk.ac.babraham.conclave.Preferences.DisplayPreferences;
import uk.ac.babraham.conclave.Utilities.FileFilters.ConclaveFileFilter;
import uk.ac.babraham.conclave.Preferences.ConclavePreferences;
import uk.ac.babraham.conclave.Vistory.Vistory;
import uk.ac.babraham.conclave.Vistory.VistoryEvent;

/**
 * ConclaveApplication represents the main Conclave GUI window and handles
 * functions which allow different parts of the UI to interact.
 */
public class ConclaveApplication extends JFrame implements ProgressListener, DataChangeListener, ProbeSetChangeListener, AnnotationCollectionListener {

	private static ConclaveApplication application;
	...
	public static void main(String[] args) {
		...
		try {
			...
			application = new ConclaveApplication();
	...
	/**
	 * Provides a static way to access the main instance of the Conclave
	...
	public static ConclaveApplication getInstance () {
		return application;
	}
	

	/**
	 * Instantiates a new Conclave application.
	 */
	private ConclaveApplication (){
...
//		setExtendedState(getExtendedState()|MAXIMIZED_BOTH);
		menu = new ConclaveMenu(this);
		setJMenuBar(menu);
		setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);
		setIconImage(new ImageIcon(ClassLoader.getSystemResource("uk/ac/babraham/Conclave/Resources/monk_logo.png")).getImage());
	...
	public void importData (DataParser parser) {
		...
		if (filter != null) {
			...
			ConclavePreferences.getInstance().setLastUsedDataLocation(chooser.getSelectedFile());
	...
	public void saveProjectAs() {
		...
		chooser.setFileFilter(new ConclaveFileFilter());
		
		int result = chooser.showSaveDialog(this);
		if (result == JFileChooser.CANCEL_OPTION) return;

		File file = chooser.getSelectedFile();
		if (! file.getPath().toLowerCase().endsWith(".ccl")) {
			file = new File(file.getPath()+".ccl");
		}
	...
	public void saveProject (File file) {
		...
		ConclaveDataWriter writer = new ConclaveDataWriter();
		...
		setTitle("Conclave ["+file.getName()+"]");
		ConclavePreferences.getInstance().addRecentlyOpenedFile(file.getAbsolutePath());
	}
	...
	public void loadProject () {
		JFileChooser chooser = new JFileChooser(ConclavePreferences.getInstance().getSaveLocation());
		chooser.setMultiSelectionEnabled(false);
		ConclavePreviewPanel previewPanel = new ConclavePreviewPanel();
	...
	public void loadProjectAndSwitchAssembly () {
		...
		ConclavePreview preview;
		
		try {
			preview = new ConclavePreview(file);
		}
	...
	public void loadProject (File file, boolean forceAssembly) {
		...
		ConclaveParser parser = new ConclaveParser(this);
	...
	private void addNewLoadedGenome(Genome g) {
		...
		// and replace it with the proper Conclave display.
		remove(welcomePanel);
		remove(mainPane);
	...
	/* (non-Javadoc)
	 * @see uk.ac.babraham.Conclave.DataTypes.DataChangeListener#dataSetRenamed(uk.ac.babraham.SConclaveDataTypes.DataSet)
	 */
	...
	/* (non-Javadoc)
	 * @see uk.ac.babraham.Conclave.DataTypes.DataChangeListener#dataSetAdded(uk.ac.babraham.Conclave.DataTypes.DataSet)
	 */
	...
	/* (non-Javadoc)
	 * @see uk.ac.babraham.Conclave.DataTypes.DataChangeListener#dataSetRemoved(uk.ac.babraham.Conclave.DataTypes.DataSet)
	 */
	...
	/* (non-Javadoc)
	 * @see uk.ac.babraham.Conclave.DataTypes.Probes.ProbeSetChangeListener#probeListAdded(uk.ac.babraham.Conclave.DataTypes.Probes.ProbeList)
	 */
	...
	/* (non-Javadoc)
	 * @see uk.ac.babraham.Conclave.DataTypes.Probes.ProbeSetChangeListener#probeListRemoved(uk.ac.babraham.Conclave.DataTypes.Probes.ProbeList)
	 */
	...
	/* (non-Javadoc)
	 * @see uk.ac.babraham.Conclave.DataTypes.DataChangeListener#probeSetReplaced(uk.ac.babraham.Conclave.DataTypes.Probes.ProbeSet)
	 */
	public void probeSetReplaced(ProbeSet probes) {
		probes.addProbeSetChangeListener(this);
		changesWereMade();
		
		Vistory.getInstance().addBlock(new VistoryEvent("New Probe Set:"+ probes.name()+" ("+probes.getAllProbes().length+" probes)",probes.justDescription()));
	}



	/* (non-Javadoc)
	 * @see uk.ac.babraham.Conclave.DataTypes.Probes.ProbeSetChangeListener#probeListRenamed(uk.ac.babraham.Conclave.DataTypes.Probes.ProbeList)
	 */
	...
	/* (non-Javadoc)
	 * @see uk.ac.babraham.Conclave.DataTypes.ProgressListener#progressComplete(java.lang.String, java.lang.Object)
	 */
	...
	/* (non-Javadoc)
	 * @see uk.ac.babraham.Conclave.DataTypes.ProgressListener#progressExceptionReceived(java.lang.Exception)
	 */
	...
	/* (non-Javadoc)
	 * @see uk.ac.babraham.Conclave.DataTypes.ProgressListener#progressUpdated(java.lang.String, int, int)
	 */
	...
	/* (non-Javadoc)
	 * @see uk.ac.babraham.Conclave.DataTypes.ProgressListener#progressCancelled()
	 */
	...
	/* (non-Javadoc)
	 * @see uk.ac.babraham.Conclave.DataTypes.ProgressListener#progressWarningReceived(java.lang.Exception)
	 */
	...
	/* (non-Javadoc)
	 * @see uk.ac.babraham.Conclave.DataTypes.Genome.AnnotationCollectionListener#annotationSetAdded(uk.ac.babraham.Conclave.DataTypes.Genome.AnnotationSet)
	 */
	...
	/* (non-Javadoc)
	 * @see uk.ac.babraham.Conclave.DataTypes.Genome.AnnotationCollectionListener#annotationSetRemoved(uk.ac.babraham.Conclave.DataTypes.Genome.AnnotationSet)
	 */
	...
	/* (non-Javadoc)
	 * @see uk.ac.babraham.Conclave.DataTypes.Genome.AnnotationCollectionListener#annotationSetRenamed(uk.ac.babraham.Conclave.DataTypes.Genome.AnnotationSet)
	 */
