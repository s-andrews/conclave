...
public class PerProbeNormalisationQuantitation extends Quantitation {
	...
	public PerProbeNormalisationQuantitation(ConclaveApplication application) {
		super(application);
	}
			...
			for (int p=0;p<probes.length;p++) {
				...
				if (useMean) {
					...
					for (int i=0;i<values.length;i++) {
						if (values[i] < 0) {
							progressWarningReceived(new ConclaveException("Negative values found when taking the sum - transformed values won't make sense"));
						}
