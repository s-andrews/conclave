 ...
 */
package uk.ac.babraham.conclave.Quantitation;
...
public class SizeFactorNormalisationQuantitation extends Quantitation {
	...
	public SizeFactorNormalisationQuantitation(ConclaveApplication application) {
		super(application);
	}
	...
	public void run() {
		...
		if (! isReady()) {
			progressExceptionReceived(new ConclaveException("Options weren't set correctly"));
		}
...
//		}
		...
		if (validProbeCount == 0) {
			progressExceptionReceived(new ConclaveException("All probes had at least one unusable (normally zero) value. Can't continue"));
		}
		...
		for (int d=0;d<data.length;d++) {
			...
			if (correctionAction == MULTIPLY) {
				if (correctionFactors[d] <= 0) {
					progressExceptionReceived(new ConclaveException("Got a zero or negative correction factor for "+data[d]+". You should probably be using ADD instead of MULTIPLY to correct"));
				}
			}
		}		
