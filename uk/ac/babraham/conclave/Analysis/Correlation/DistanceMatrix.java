 ...
 */
package uk.ac.babraham.conclave.Analysis.Correlation;
...
public class DistanceMatrix implements Runnable, Cancellable {
	...
	public float getCorrelationForStores(DataStore store1, DataStore store2) throws ConclaveException {
		
		if (! calculationComplete) {
			throw new ConclaveException("Correlation calculation not yet complete");
		}
		...
		if (st1 == -1 || st2 == -1) {
			throw new ConclaveException("Attempted to get correlation for a store not in the distance matrix");
		}
		
		return getCorrelationForStoreIndices(st1, st2);
	}
	
	public float getMinCorrelation () throws ConclaveException {
	...
	public float getMaxCorrelation () throws ConclaveException {
	...
	public float getCorrelationForStoreIndices (int st1, int st2) throws ConclaveException {
		...
		if (st1 >= stores.length || st2 >= stores.length) {
			throw new ConclaveException("Store index was larger than the list of stores");
		}
