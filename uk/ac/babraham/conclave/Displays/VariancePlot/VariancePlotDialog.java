 ...
 */
package uk.ac.babraham.conclave.Displays.VariancePlot;
...
public class VariancePlotDialog extends JDialog implements ActionListener, ChangeListener {
	...
	public VariancePlotDialog (DataCollection collection) {
		super(ConclaveApplication.getInstance(),"Variation Plot");
		...
		if (quantitatedStores.size() == 0) {
			JOptionPane.showMessageDialog(ConclaveApplication.getInstance(), "Can't draw a plot - no quanitated replicate sets exist", "Can't draw plot", JOptionPane.INFORMATION_MESSAGE);
			return;
		}
