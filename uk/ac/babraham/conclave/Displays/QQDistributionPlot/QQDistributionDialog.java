...
public class QQDistributionDialog extends JDialog implements ActionListener, ChangeListener {
	...
	public QQDistributionDialog (DataStore [] stores, ProbeList list) throws ConclaveException {
		super(ConclaveApplication.getInstance(),"QQ Distribution Graph ["+list.name()+"]");
		graphPanel = new QQDistributionPanel(stores, list);
		setup();
	}

	public QQDistributionDialog (DataStore store, ProbeList [] lists) throws ConclaveException {
		super(ConclaveApplication.getInstance(),"QQ Distribution Graph ["+store.name()+"]");
