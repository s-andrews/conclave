...
public class RNAQCResultsDialog extends JDialog implements SampleSelectionListener {
	...
	public RNAQCResultsDialog (RNAQCResult result) {
		...
		super(ConclaveApplication.getInstance(),"RNA-Seq QC Report");
		...
		removeButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent ae) {
				...
				if (answer == JOptionPane.YES_OPTION) {
					...
					DataStore [] visibleStores = ConclaveApplication.getInstance().drawnDataStores();
					...
					if (storesToKeep.size() < visibleStores.length) {
						...
						ConclaveApplication.getInstance().setDrawnDataStores(storesToKeep.toArray(new DataStore[0]));
					}
				}
