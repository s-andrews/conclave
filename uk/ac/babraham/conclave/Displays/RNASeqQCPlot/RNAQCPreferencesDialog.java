 ...
 */
package uk.ac.babraham.conclave.Displays.RNASeqQCPlot;
...
public class RNAQCPreferencesDialog extends JDialog implements ActionListener, ProgressListener {
	...
	public RNAQCPreferencesDialog(DataCollection collection, DataStore [] stores)  {
		super(ConclaveApplication.getInstance(),"RNA-Seq QC Plot");
		
		if (stores.length == 0) {
			JOptionPane.showMessageDialog(ConclaveApplication.getInstance(), "There are no visible data stores", "Can't draw plot", JOptionPane.INFORMATION_MESSAGE);
			return;
		}
		
		if (collection.genome().annotationCollection().listAvailableFeatureTypes().length == 0) {
			JOptionPane.showMessageDialog(ConclaveApplication.getInstance(), "You don't have any annotation available", "Can't draw plot", JOptionPane.INFORMATION_MESSAGE);
			return;
		}
