 ...
 */
package uk.ac.babraham.conclave.Displays.Domainogram;
...
import uk.ac.babraham.conclave.Displays.GenomeViewer.ChromosomeScale;
...
public class DomainogramDialog extends JDialog implements ChangeListener, ActionListener {
	...
	public DomainogramDialog (ProbeList probes, DataStore [] data, int [] levels, boolean ignoreBlankChromosomes, boolean trimEnds) {
		super(ConclaveApplication.getInstance(),"Domainogram for "+probes.name());
		...
		int longestChromosome = ConclaveApplication.getInstance().dataCollection().genome().getLongestChromosomeLength();
