 ...
 */
package uk.ac.babraham.conclave.Displays.DataStoreTree;
...
public class DataStoreTreeDialog extends JDialog implements ProgressListener {
	...
	public DataStoreTreeDialog (ProbeList probeList, DataStore [] stores) {
		...
		super(ConclaveApplication.getInstance(),"Data Store Tree ["+probeList.name()+"]");
		this.stores = stores;
	...
	public void progressComplete(String command, Object result) {
		...
		createRepSetsButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				for (int s=0;s<splitStores.length;s++) {
					ConclaveApplication.getInstance().dataCollection().addReplicateSet(new ReplicateSet("Cluster Group "+(s+1), splitStores[s]));
				}
		...
		reorderStoresButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				ConclaveApplication.getInstance().setDrawnDataStores(orderedStores);
