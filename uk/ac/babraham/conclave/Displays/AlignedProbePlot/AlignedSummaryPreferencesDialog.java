 ...
 */
package uk.ac.babraham.conclave.Displays.AlignedProbePlot;
...
import uk.ac.babraham.conclave.DataTypes.Sequence.QuantitationStrandType;
...
public class AlignedSummaryPreferencesDialog extends JDialog implements ActionListener {
	...
	public AlignedSummaryPreferencesDialog (ProbeList [] lists, DataStore [] stores) {
		super(ConclaveApplication.getInstance(),"Aligned Probe Preferences");
		this.lists = lists;
		this.stores = stores;
		
		if (stores.length == 0) {
			JOptionPane.showMessageDialog(ConclaveApplication.getInstance(), "<html>You need at least one data store to be visible to draw this plot<br>Add at least one store to the chromosome view, then try again", "Can't draw plot", JOptionPane.INFORMATION_MESSAGE);
			return;
		}
