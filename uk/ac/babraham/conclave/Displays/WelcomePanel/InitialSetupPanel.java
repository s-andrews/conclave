...
public class InitialSetupPanel extends JDialog implements ActionListener {
	...
	public InitialSetupPanel () {
		...
		super(ConclaveApplication.getInstance(),"Initial Conclave Setup");
		setModal(true);
		...
		ImageIcon logo = new ImageIcon(ClassLoader.getSystemResource("uk/ac/babraham/Conclave/Resources/bi_logo.png"));
		ImageIcon monk = new ImageIcon(ClassLoader.getSystemResource("uk/ac/babraham/Conclave/Resources/monk100.png"));

		topPanel.add(new JLabel("",logo,JLabel.CENTER),BorderLayout.WEST);
		topPanel.add(new JLabel("",monk,JLabel.CENTER),BorderLayout.EAST);
		
		JLabel program = new SmoothJLabel("Welcome to Conclave",JLabel.CENTER);
		...
		JLabel explainLabel = new JLabel(
				"<html>To get started with Conclave we need to set up a couple of things<br><br>"
				+ " 1) A 'cache' folder where Conclave can temporarily write data whilst it's running<br>"
		...
		cacheFolderField.setText(System.getProperty("user.home")+"/conclave_cache");
		...
		c.gridx=1;
		c.weightx=0.5;
		genomeFolderField = new JTextField(System.getProperty("user.home")+"/conclave_genomes");
		...
		JButton goButton = new JButton("Start Using Conclave");
	...
	public void actionPerformed(ActionEvent ae) {
		...
		if (ae.getActionCommand().equals("cache")) {
			...
			ConclavePreferences p = ConclavePreferences.getInstance();
