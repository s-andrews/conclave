 ...
 */
package uk.ac.babraham.conclave.Displays.WelcomePanel;
...
import uk.ac.babraham.conclave.Dialogs.ProgressRecordDialog;
...
import uk.ac.babraham.conclave.Network.GenomeUpgrader;
import uk.ac.babraham.conclave.Network.UpdateChecker;
import uk.ac.babraham.conclave.Network.DownloadableGenomes.DownloadableGenomeSet;
import uk.ac.babraham.conclave.Network.DownloadableGenomes.GenomeAssembly;
import uk.ac.babraham.conclave.Preferences.ConclavePreferences;
import uk.ac.babraham.conclave.R.RProgressListener;
import uk.ac.babraham.conclave.R.RScriptRunner;
import uk.ac.babraham.conclave.R.RVersionTest;
import uk.ac.babraham.conclave.Utilities.TempDirectory;
import uk.ac.babraham.conclave.Utilities.Templates.Template;

/**
 * This panel is displayed when the program first starts.
 * It shows information about the current SeqMonk install
 */
public class ConclaveInformationPanel extends JPanel implements Runnable, ActionListener {
	...
	private ImageIcon infoIcon = new ImageIcon(ClassLoader.getSystemResource("uk/ac/babraham/Conclave/Resources/StatusIcons/information.png"));

	/** The error icon. */
	private Icon errorIcon = new ImageIcon(ClassLoader.getSystemResource("uk/ac/babraham/Conclave/Resources/StatusIcons/error.png"));

	/** The warning icon. */
	private Icon warningIcon = new ImageIcon(ClassLoader.getSystemResource("uk/ac/babraham/Conclave/Resources/StatusIcons/warning.png"));

	/** The tick icon. */
	private Icon tickIcon = new ImageIcon(ClassLoader.getSystemResource("uk/ac/babraham/Conclave/Resources/StatusIcons/tick.png"));
	 ...
	 */
	public ConclaveInformationPanel (ConclaveApplication application) {
	...
	protected void populatePanel () {
		...
		File tempDir = ConclavePreferences.getInstance().tempDirectory();
		...
		if (tempDir == null) {
			...
			try {
				File tempFile = File.createTempFile("seqmonk_test_data", ".temp", ConclavePreferences.getInstance().tempDirectory());
		...
		if (ConclavePreferences.getInstance().customGenomeBaseUsed()) {
			// They're using a custom genomes folder
			File gb = null;
			try {
				gb = ConclavePreferences.getInstance().getGenomeBase();
			} 
		...
		if (ConclavePreferences.getInstance().checkForUpdates()) {
	...
	public void run () {
		...
		try {
			...
			if (UpdateChecker.isUpdateAvailable()) {
				...
				programUpdateLabel.setIcon(warningIcon);
				programUpdateLabelText.setText("A newer version of Conclave (v"+latestVersion+") is available");
			}
			else {
				if (ConclaveApplication.VERSION.contains("devel")) {
					programUpdateLabel.setIcon(warningIcon);
					programUpdateLabelText.setText("You are running a current development version of Conclave");					
				}
				else {
					programUpdateLabel.setIcon(tickIcon);
					programUpdateLabelText.setText("You are running the latest version of Conclave");
				}
			}
		}
		catch (ConclaveException e) {
			programUpdateLabel.setIcon(errorIcon);
			programUpdateLabelText.setText("Failed to check for Conclave updates");	
			e.printStackTrace();
		}
		...
		try {
			String rVersion = RVersionTest.testRVersion(ConclavePreferences.getInstance().RLocation());
			...
			if (!RVersionTest.hasRDependencies()) {
				...
				rLabelText.setText("Found valid R version ("+rVersion+") at '"+ConclavePreferences.getInstance().RLocation()+"'");

			}
						
		}
		catch (IOException ioe) {
			if (ConclavePreferences.getInstance().RLocation().equals("R")) {
				...
				rLabelText.setText("Couldn't find a valid R installation at "+ConclavePreferences.getInstance().RLocation()+" "+ioe.getMessage());
				setRLocationButton.setVisible(true);
			}
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getActionCommand().equals("set_temp_dir")) {
			...
			if (chooser.showOpenDialog(this) == JFileChooser.APPROVE_OPTION) {
				ConclavePreferences.getInstance().setTempDirectory(chooser.getSelectedFile());
				try {
					ConclavePreferences.getInstance().savePreferences();
					populatePanel();
				} 
				catch (IOException ioe) {
			...
			chooser.setFileSelectionMode(JFileChooser.DIRECTORIES_ONLY);
			if (chooser.showOpenDialog(this) == JFileChooser.APPROVE_OPTION) {
				ConclavePreferences.getInstance().setGenomeBase(chooser.getSelectedFile());
				try {
					ConclavePreferences.getInstance().savePreferences();
					populatePanel();
				} 
			...
			if (autoDetect != null) {
				...
				if (answer == JOptionPane.YES_OPTION) {
					ConclavePreferences.getInstance().setRLocation(autoDetect);
					try {
						ConclavePreferences.getInstance().savePreferences();
						populatePanel();
						return;
			...
			if (chooser.showOpenDialog(this) == JFileChooser.APPROVE_OPTION) {
				ConclavePreferences.getInstance().setRLocation(chooser.getSelectedFile().getAbsolutePath());
				try {
					ConclavePreferences.getInstance().savePreferences();
					populatePanel();
				} 
			...
			Thread t = new Thread(new Runnable() {
				public void run() {
					try {
						...
						Template template = new Template(ClassLoader.getSystemResource("uk/ac/babraham/Conclave/create_local_library.r"));
						...
						// Get the template script
						Template template2 = new Template(ClassLoader.getSystemResource("uk/ac/babraham/Conclave/load_required_modules.r"));
			...
			Thread t = new Thread(new Runnable() {
				...
				public void run() {
					...
					File [] tempFiles = ConclavePreferences.getInstance().tempDirectory().listFiles();
