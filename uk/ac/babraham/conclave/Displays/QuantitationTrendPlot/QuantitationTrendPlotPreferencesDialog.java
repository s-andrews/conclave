...
public class QuantitationTrendPlotPreferencesDialog extends JDialog implements ActionListener, ProgressListener {
	...
	public QuantitationTrendPlotPreferencesDialog (DataCollection collection, ProbeList [] probes, DataStore [] stores) {
		super(ConclaveApplication.getInstance(),"Quantitation Trend Preferences");
		this.probes = probes;
		...
		if (validStores.size() == stores.length) {
			...
			JOptionPane.showMessageDialog(ConclaveApplication.getInstance(), "Can't draw this plot. None of your visible DataStores were quantitated", "Error drawing plot", JOptionPane.ERROR_MESSAGE);
			return;
		}
		else {
			// Some stores were bad
			JOptionPane.showMessageDialog(ConclaveApplication.getInstance(), "Some unquantitated stores were removed", "Warning drawing plot", JOptionPane.WARNING_MESSAGE);
