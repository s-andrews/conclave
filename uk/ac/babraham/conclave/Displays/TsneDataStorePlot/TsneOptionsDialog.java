...
public class TsneOptionsDialog extends JDialog implements ActionListener {
	...
	public TsneOptionsDialog (ProbeList probes, DataStore [] stores) {
		...
		super(ConclaveApplication.getInstance(),"Tsne Options");
		
		
		// We can't run Tsne with fewer than 4 stores.
		if (stores.length < 4) {
			JOptionPane.showMessageDialog(ConclaveApplication.getInstance(), "<html>Sorry - you can't run Tsne with fewer than 4 datasets.<br>You only had "+stores.length+" valid data stores</html>", "Can't run Tsne", JOptionPane.INFORMATION_MESSAGE);
			return;
		}
