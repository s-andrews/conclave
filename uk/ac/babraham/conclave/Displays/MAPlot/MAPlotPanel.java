 ...
 */
package uk.ac.babraham.conclave.Displays.MAPlot;
 ...
 * regions.
 */
public class MAPlotPanel extends JPanel implements Runnable, MouseMotionListener, MouseListener {
	 ...
	 */
	public MAPlotPanel (DataStore xStore, DataStore yStore, ProbeList probeList, ProbeList [] subLists, boolean commonScale, int dotSize) {
	...
	public void run() {
		...
		try {
			...
			for (int p=0;p<probes.length;p++) {
				...
				yData[p]=xStore.getValueForProbe(probes[p])-yStore.getValueForProbe(probes[p]);
			...
			if (commonScale) {
				// Make y symmetrical around 0
				if (Math.abs(minValueY)>Math.abs(maxValueY)) {
					minValueY = 0-Math.abs(minValueY);
					maxValueY = Math.abs(minValueY);
				}
				else {
					minValueY = 0-Math.abs(maxValueY);
					maxValueY = Math.abs(maxValueY);
				}
			}
