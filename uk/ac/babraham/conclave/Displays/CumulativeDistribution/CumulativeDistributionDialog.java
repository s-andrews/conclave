 ...
 */
package uk.ac.babraham.conclave.Displays.CumulativeDistribution;
...
public class CumulativeDistributionDialog extends JDialog implements ActionListener, ChangeListener {
	...
	private CumulativeDistributionPanel graphPanel;
	private JSlider scaleSlider;

	public CumulativeDistributionDialog (DataStore [] stores, ProbeList list) throws ConclaveException {
		super(ConclaveApplication.getInstance(),"Cumulative Distribution Graph ["+list.name()+"]");
		graphPanel = new CumulativeDistributionPanel(stores, list);
		setup();
	}

	public CumulativeDistributionDialog (DataStore store, ProbeList [] lists) throws ConclaveException {
		super(ConclaveApplication.getInstance(),"Cumulative Distribution Graph ["+store.name()+"]");
		graphPanel = new CumulativeDistributionPanel(store, lists);
		setup();
	}
