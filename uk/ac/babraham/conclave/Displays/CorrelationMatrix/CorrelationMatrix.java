 ...
 */
package uk.ac.babraham.conclave.Displays.CorrelationMatrix;
...
import uk.ac.babraham.conclave.Analysis.Correlation.DistanceMatrix;
...
public class CorrelationMatrix extends JDialog implements ProgressListener, ActionListener {
	...
	public CorrelationMatrix (DataStore [] stores, ProbeList probes) {
		super(ConclaveApplication.getInstance(),"Correlation table ["+probes.name()+"]");
		matrix = new DistanceMatrix(stores,probes.getAllProbes());
		matrix.addProgressListener(this);
		matrix.addProgressListener(new ProgressDialog(ConclaveApplication.getInstance(), "Correlation Calculation",matrix));
