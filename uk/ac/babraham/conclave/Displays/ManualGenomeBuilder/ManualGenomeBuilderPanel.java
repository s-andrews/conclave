 ...
 */
package uk.ac.babraham.conclave.Displays.ManualGenomeBuilder;
...
import uk.ac.babraham.conclave.Utilities.FileFilters.FastaFileFilter;
import uk.ac.babraham.conclave.Utilities.FileFilters.GFFFileFilter;
...
public class ManualGenomeBuilderPanel extends JPanel implements ActionListener, KeyListener {
	...
	public void actionPerformed(ActionEvent ae) {
		...
		if (ae.getActionCommand().equals("add")) {
			...
			try {
			speciesFolder = new File(ConclavePreferences.getInstance().getGenomeBase().getAbsolutePath()+"/"+speciesName);
		...
		public void run () {
			...
			try {
				...
				while ((line = br.readLine()) != null) {
					...
					if (fields.length < 5) {
						pd.progressWarningReceived(new ConclaveException("No enough fields on line '"+line+"'"));
						continue;
					}
